...
if (isset($_POST['submit'])) {
		  ...
		  require 'C:/xampp/htdocs/incl/db_conn.php';
			...
			$password = $_POST['password'];
			    			...
			    			//link header return error
			          		header("Location: ../../admin/pages/login.php?error=emptyfields");
			          		exit();
			  				} else {
					  		$l_sql = "SELECT * FROM `users` WHERE `email` = ? AND roles = 1";
						  	//db connection
						  	$stmt = mysqli_stmt_init($dbc);
						  	//we chack if the query can perform // the stament work essencially
						  	if (!mysqli_stmt_prepare($stmt, $l_sql)){
								header("Location: ../../admin/pages/login.php?error=SQLerror");
						        exit();
							  	}else{
									...
									$result = mysqli_stmt_get_result ($stmt);
							  		if ($row = mysqli_fetch_assoc($result)){
							  			...
							  			//we get row data from the database
											...
											header("Location: ../../admin/pages/login.php?error=nomember");
						  	 				exit();
							  			}
							  			$passcheck= password_verify($password, $row['password']);
							  			if ($passcheck == false) {
							  				header("Location: ../../admin/pages/login.php?error=wrongpass");
						  	 				exit();
							  				...
							  				header("Location: ../../admin/index.php?success=loggedin");
						  	 				exit();
