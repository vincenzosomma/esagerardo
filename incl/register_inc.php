...
if (isset($_POST['submit'])) {
  ...
  if (empty($fname) || (empty($lname)) || (empty($email)) || (empty($phone)) || (empty($address)) || (empty($postcode)) || empty($password) || empty($confirmpassword))
  {
    //link header return error
          header("Location: ../pages/registration.php?error=emptyfields&=" . $fname);
     
          exit();
  } elseif (!preg_match("/^[a-zA-Z0-9]*/", $fname)) {
    //       //control charateers on the fname...
          header("Location: ../pages/registration.php?error=invalidname&fname=" . $fname);
          exit();
  } elseif (!preg_match("/^[a-zA-Z0-9]*/", $lname)) {
    //       //control charateers on the lname...
          header("Location: ../pages/registration.php?error=invalidsurname&lname=" . $lname);
          $message = "Invalid Charatcter";
          exit();
} elseif (!preg_match("/^[0-9]*/", $phone)) {
    //       //control charateers on the phone...
          header("Location: ../pages/registration.php?error=invalidphone&phone=" . $fname);
          exit();
  } elseif ($password != $confirmpassword) {
    // check if the passwords are the same
        header("Location: ../pages/registration.php?error=passwordsdonotmatch&fname=" . $fname);
        exit();
  } else {
          ...
          header("Location: ../pages/registration.php?error=SQLerror1");
          exit();
        }else {
          ...
          if ($rowcount > 0){
            header("Location: ../pages/registration.php?error=alreadymember");
            exit();
          }else{
            ...
            //VALUES(?, ?) placed orders
              $stmt = mysqli_stmt_init($dbc);
              if (!mysqli_stmt_prepare($stmt, $r_sql)) {
                  header("Location: ../pages/registration.php?error=SQLerror2");
                  exit();
              } else {
              /* we have insert password an "s" because we insert the data into the SQLiteDatabase
                ...
                mysqli_stmt_execute($stmt);
                header("Location: ../pages/login.php?error=registred");
                exit();
